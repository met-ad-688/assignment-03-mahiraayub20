...
# Step 1: Filter dataset (remove missing or zero salaries)
df = df[df["SALARY_FROM"].notnull()]
df = df[df["SALARY_FROM"] > 0]

# Step 2: Create boxplot grouped by industry and employment type
...
plt.title("Salary Distribution by Industry and Employment Type", fontsize=14, weight="bold")
plt.xlabel("Industry (NAICS2)", fontsize=12)
plt.ylabel("Salary (From)", fontsize=12)
